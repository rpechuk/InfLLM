...
class VectorTensor:
    ...
    # MILLAN GUESS: tensor is a matrix of "current token" query vectors
    # and self.data is a bunch of representatives?
    # Or maybe this is happening in get_batched_topk
    def get_topk(self, tensor: torch.Tensor, topk): # inner product
        ...
        logits = self.data[:self.length] * tensor[:, None].T
...
def quest_get_topk(block_k_minima, block_k_maxima, tensor, topk):
    self = block_k_minima   # Stupid hack to make this function work outside of a class
    tensor = tensor.to(self.data.device)
    min_product = block_k_minima.data[:self.length] * tensor[:, None].T
    max_product = block_k_maxima.data[:self.length] * tensor[:, None].T
    # element-wise maximum
    mixed_product = torch.maximum(min_product, max_product)
    logits = torch.sum(mixed_product, dim=1, keepdim=False)
...
class ContextManager:
    ...
    def init(
        ...
        if self.faiss:
            ...
            self.block_k_minima = [VectorTensor(
                dim_head * self.unit_size, global_k.dtype
            ) for _ in range(self.num_units)]
            self.block_k_maxima = [VectorTensor(
                dim_head * self.unit_size, global_k.dtype
            ) for _ in range(self.num_units)]
    ...
    def calc_block_topk(
        ...
        if not self._use_chunk_topk:
            ...
            # Millan : This is taking the mean of the "current token" query vectors
            # Note that the dot product of the mean of current tokens and the mean
            # of representatives for a particular block is the same as 
            ...
            for u in range(self.num_units):
                # MILLAN GUESS:
                # This is getting the top k most relevant blocks to the current tokens,
                # whose query vectors are stored in global_h_q[u].
                ########################################## WITHOUT QUEST ##########################
                # ret.append(self.block_k[u].get_topk(global_h_q[u], self.topk))
                ########################################## WITH QUEST #############################
                ret.append(quest_get_topk(self.block_k_minima[u], self.block_k_maxima[u], global_h_q[u], self.topk))
    ...
    # MILLAN: This function is not used in infinitebench
    ...
    def append_global(
        ...
        while global_remainder_len - self.block_size >= self.n_local:
            ...
            global_block_k_minima = global_block_k_minima.reshape(self.num_units, self.unit_size * self.dim_head)
            global_block_k_minima = global_block_k_minima[:, None, :]
            ...
            for u in range(self.num_units):
                ...
                ################################# NEW, QUEST #########################################
                self.block_k_minima[u].append(global_block_k_minima[u])
                self.block_k_maxima[u].append(global_block_k_maxima[u])
                ######################################################################################
